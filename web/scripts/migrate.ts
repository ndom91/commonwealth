...
/* `slug` is better-auth's, not ours — the organization plugin requires it and
   `projects` is the table it is pointed at. See `lib/auth.ts`.
 *
 ...
 * from `/people`. Keying on the name would mean a renamed project no longer
 ...
 * nobody chose and which became visible on the rail the moment projects went
 * plural; existing instances keep whatever they have. It names the *project*,
...
const [project] = await client<{ id: string }[]>`
  INSERT INTO projects (name, slug) VALUES ('Core team', 'default')
  ON CONFLICT (slug) DO UPDATE SET slug = EXCLUDED.slug
  RETURNING id
`;
if (!project) throw new Error('Unable to create default project');
/* Checked against the default project alone, and relied on instance-wide.
   That holds because there is one model for the whole instance: every other
   project gets its configuration copied from an existing one by
   `createProject`, so they cannot disagree. If a model per project is ever
...
>`
  SELECT embedding_model, embedding_dimensions FROM index_configuration WHERE project_id = ${project.id}
`;
if (!indexConfiguration) {
  await client`
    INSERT INTO index_configuration (project_id, embedding_model, embedding_dimensions)
    VALUES (${project.id}, ${embeddingModel}, 1024)
  `;
} else if (
...
await db
  .insert(member)
  .values({ organizationId: project.id, userId: admin.id, role: 'admin' })
  .onConflictDoNothing();
await client.end();
