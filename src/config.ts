...
const positiveInt = (fallback: number) =>
  v.optional(v.pipe(v.string(), v.transform(Number), v.number(), v.integer(), v.minValue(1)), String(fallback));
...
export function loadConfig(): Config {
  ...
  const problems = result.issues
    ...
    .join(", ");
  throw new Error(`Invalid environment configuration: ${problems}. Set these on the service and restart.`);
}
