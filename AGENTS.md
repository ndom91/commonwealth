...
Every route under `web/src/app/p/$slug/` is one corpus. The slug in the path is
...
`mcp-server/src/` needs none of this. It has always scoped to `actor.projectId`, so agents
...
administrator, project, and embedding configuration. Every future migration
